...
def execute(filters=None):
    ...
    for sle in sl_entries:
        # Set stock_entry_type only for Stock Entry voucher type
        if sle.voucher_type == "Stock Entry":
            sle.stock_entry_type = frappe.get_value("Stock Entry", sle.voucher_no, "stock_entry_type")
        else:
            sle.stock_entry_type = ""  # Empty string for non-Stock Entry vouchers
...
def get_stock_ledger_entries(filters, items):
    ...
    query = (
        frappe.qb.from_(sle)
        .left_join(se)
        .on((sle.voucher_no == se.name) & (sle.voucher_type == "Stock Entry"))
        .left_join(warehouse)
        .on(sle.warehouse == warehouse.name)
        .left_join(subcontractor)
        .on(warehouse.custom_subcontractor == subcontractor.name)
        .left_join(project)  # Join with Project
        .on(warehouse.custom_project == project.name)  
        .select(
            sle.item_code,
    ...
    if filters.get("subcontractor"):
        query = query.where(subcontractor.name == filters.get("subcontractor"))
    ...
    # Modified project filter to check both SLE project and warehouse custom_project
    if filters.get("project"):
        query = query.where(
            warehouse.custom_project == filters.get("project")
        )
    ...
    # Post-process results to set project field
    for row in results:
        # Use warehouse custom_project if SLE project is empty
        if not row.get("project"):
            row["project"] = row.get("custom_project")
        
        # Remove the custom_project field as it was only used for processing
        if "custom_project" in row:
            del row["custom_project"]
