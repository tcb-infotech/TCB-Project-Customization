...
def prepare_data(data, so_elapsed_time, filters):
	...
	for row in data:
		...
		#print()
		if row.quotation:
			sale_order_all_items = frappe.get_doc("Sales Order",row.sales_order).items
			all_so_items_list = []
			new_data = []
			for item in sale_order_all_items:
				all_so_items_list.append(item.item_code)
				# pri
    # nt('-----items -----------=============--',all_so_items_list)
			# item_matched = False
			quotation_doc = frappe.get_doc("Quotation",row.quotation)
			for item in quotation_doc.items:
				#print('--------outside-----here is the tiem ',item.item_code)
				if item.item_code == row.item_code:
					#print('-----inside the if condition --------here is the tiem ',item.item_code)
					# #print('-----------',item)
					row["quoted_item"] = item.item_code
					row["quotation"] = row.quotation
					# item_matched = True
				else:
					# for so_item in sale_order_all_items:
					if item.item_code not in all_so_items_list:
							#print('--------------------')
							new_row = {}
							# new_row = {}
							new_row["item_code"] = ""
							new_row["billed_amount"] = 00
							new_row["pending_amount"] = 00
							new_row["billed_qty"] = 00
							new_row["qty"] = 00
							new_row["delay"] = 00
							new_row["status"] = ""
							new_row["quotation"] = row.quotation

							new_row["quoted_item"] = item.item_code
							new_data.append(new_row)
			#print('---------=====this is new data ===',new_data)
	if new_data:
		data.extend(new_data)
			# #print('----------this this data.quotaion - ',(row if row else ""))
			# row['quotation'] = row.quotation
		# else:
		# 	#print('----------this this daquotationta.quotaion - ',data.quotation if data.quotation else "")
